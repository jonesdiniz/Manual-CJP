...
\begin{verbatim}
OPÇÃO 1 (Com Fator Prev - se vantajoso):
RMI = SB × Fator Previdenciário

OPÇÃO 2 (Sem Fator Prev):
RMI = SB × 100%
...
Coeficiente = 60% + 2% × (Tempo Total - 20H/15M)
Máximo = 100%
\end{verbatim}
...
RMI = R$ 6.500 × 1,00 = R$ 6.500,00 (INTEGRAL)
\end{verbatim}
...
\hline
Direito Adquirido (Tempo) & 80\% maiores (Jul/94→DER) & 100\% ou Fator Prev (o maior) & Teto INSS \\
\hline
Direito Adquirido (Idade) & 80\% maiores (Jul/94→DER) & 70\% + 1\% por ano (max 100\%) & Teto INSS \\
\hline
Pontos (Art. 15) & 100\% todos (Div≥108) & 60\% + 2\% × (tempo-20/15) (max 100\%) & Teto INSS \\
\hline
Pedágio 50\% (Art. 17) & 80\% maiores (Div≥60\%) & Fator Previdenciário (pode ser $<$ 100\%) & Teto INSS \\
\hline
Pedágio 100\% (Art. 20) & 80\% maiores (Div≥60\%) & 100\% (INTEGRAL) & Teto INSS \\
\hline
Idade Progressiva (Art. 18) & 100\% todos (Div≥108) & 60\% + 2\% × (tempo-20/15) (max 100\%) & Teto INSS \\
\hline
Permanente (Art. 19) & 100\% todos (Div≥108) & 60\% + 2\% × (tempo-20/15) (max 100\%) & Teto INSS \\
\hline
\end{tabular}
\end{center}

{Teto INSS 2026: R\$ 8.537,55* (confirmar Portaria janeiro/2026)}\\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item[SIM] → Verificar regra de transição
        \item[NÃO] → PBC = 100\% todos
    \end{itemize}
    \item[$\square$] A regra escolhida é Pedágio 50\% ou 100\%?
    \begin{itemize}
        \item[SIM] → PBC = 80\% maiores
        \item[NÃO] → PBC = 100\% todos
    \end{itemize}
    \item[$\square$] O PBC inicia em Jul/1994?
    \begin{itemize}
        \item[SIM] → Correto
        \item[NÃO] → ERRO! (salvo casos especiais)
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Pós-Reforma: 60\% + 2\% × (tempo - 20H/15M)
        \item Pré-Reforma (Idade): 70\% + 1\% × tempo
    \end{itemize}
...
\begin{itemize}
    \item[$\square$] Fórmula completa está correta? (RMI = SB × Coef/Fator)
...
\begin{itemize}
    \item[$\square$] Teto INSS (2026): R\$ 8.537,55*
    \begin{itemize}
        \item Se RMI $>$ Teto → Limitar a R\$ 8.537,55
    \end{itemize}
    \item[$\square$] Piso: 1 Salário Mínimo (R\$ 1.621,00 em 2026)
    \begin{itemize}
        \item Se RMI $<$ Piso → Elevar a R\$ 1.621,00
    \end{itemize}
...
\textbf{Impacto médio:} -R\$ 400-900/mês
...
\textbf{Impacto médio:} -R\$ 200-600/mês
...
Fundamentação: Lei 14.331/2022 c/c Art. 26, § 6º, EC 103/2019
\end{verbatim}

\subsection{Erro \#3: Coeficiente Calculado Errado}

\textbf{Frequência:} 25\% dos casos\\
\textbf{Impacto médio:} -R\$ 300-800/mês
...
\textbf{Impacto médio:} Variável (R\$ 500-2.000/mês)
...
Fundamentação: Art. 3º, EC 103/2019 (direito de opção)
\end{verbatim}

\subsection{Erro \#6: Tempo Especial Não Convertido}

\textbf{Frequência:} 10\% dos casos\\
\textbf{Impacto médio:} -R\$ 800-1.500/mês
...
\textbf{3. FUNDAMENTAÇÃO JURÍDICA}
\begin{itemize}
