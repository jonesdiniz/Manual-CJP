...
Diagnóstico → RAC → CNIS Perfeito → Pedido Benefício

\textbf{CENÁRIO 2 (REATIVO):}\\
Benefício Concedido → Diagnóstico → Revisão
...
\begin{itemize}
    \item Se provas já estavam no PA → DIB na DER
    \item Se provas não foram levadas → DIB na citação
\end{itemize}
...
\textbf{1º - DOCUMENTOS ORIGINAIS OFICIAIS}\\
(CTPS, PPP, Certidões, Holerites originais)

\textbf{2º - DOCUMENTOS COMPLEMENTARES}\\
(Extrato FGTS, Ficha de Registro, Contracheque)

\textbf{3º - PROVAS TESTEMUNHAIS/INDIRETAS}\\
...
\begin{enumerate}
    ...
    \item Selecione: Novo Pedido → Atualizar Meus Dados → Acerto de CNIS
...
\begin{itemize}
    \item ``Em análise'' → Aguardando
    \item ``Exigência'' → INSS solicitou documentos adicionais (responda em 30 dias!)
    \item ``Deferido'' → Acerto realizado
    \item ``Indeferido'' → Cabe recurso em 30 dias
\end{itemize}
