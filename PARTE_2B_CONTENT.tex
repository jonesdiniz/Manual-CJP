...
\begin{itemize}
    \item[\xmark] Vínculo ignorado → Tempo não conta
...
\begin{itemize}
    \item[\cmark] Você JÁ tem a sentença → Não precisa provar novamente
    \item[\cmark] Valores reconhecidos judicialmente → Presunção absoluta
...
\textbf{Cliente:} Pedro, aposentado desde 2020 com RMI R\$ 3.100/mês\\
\textbf{Processo trabalhista (2022) reconheceu:}
\begin{itemize}
    \item 3 anos de vínculo não registrado (2005-2008)
    \item Salários de R\$ 3.500/mês (vs. R\$ 1.200 no CNIS)
\end{itemize}

\textbf{IMPACTO DA AVERBAÇÃO:}
\begin{itemize}
    \item Tempo adicional: +3 anos
    \item Nova RMI: R\$ 3.710/mês (+R\$ 610)
    \item Retroativo: R\$ 29.280
    \item Ganho vitalício: R\$ 146.400
\end{itemize}
...
\begin{enumerate}
    ...
    \item Demonstre cálculo: salário × alíquota = valor pago
...
\textbf{III.} Em caso de DÚVIDA → Conclusão FAVORÁVEL ao segurado
...
O segurado enquadrado na regra de transição do art. 3º da Lei 9.876/1999 NÃO pode optar pela regra definitiva do art. 29, I e II, da Lei 8.213/1991.
\end{armadilha}
...
\begin{estrategiaCJP}
\textbf{MATRIZ CJP DE PRIORIZAÇÃO}

\textbf{PRIORIDADE 1 (Resolver IMEDIATAMENTE):}
\begin{itemize}
    \item PDESFAZ-AJ-EC103 → Bloqueia concessão
    \item PSC-MEN-SM-EC103 → Período não conta
    \item PEMP-CAD → Vínculo ignorado
\end{itemize}

\textbf{PRIORIDADE 2 (Resolver ANTES de pedir benefício):}
\begin{itemize}
    \item PEXT → Tempo não conta
    \item PREC-MENOR-MIN → PBC reduzido
    \item PREC-FBR → Base zerada
\end{itemize}

\textbf{PRIORIDADE 3 (Oportunidade de aumento):}
\begin{itemize}
    \item PVIN-REC-PROC-TRAB → Pode adicionar tempo/salário
    \item IEAN → Pode converter tempo especial
\end{itemize}

\textbf{PRIORIDADE 4 (Monitorar):}
\begin{itemize}
    \item ACNIS, AHOM, APEND → Aguardar
    \item ICAR → Planejar complementação
    \item IPREVI → Avaliar CTC
\end{itemize}

\textbf{IGNORAR:} IAUTVID → Tese cancelada
\end{estrategiaCJP}

\section{Casos Práticos: 3 CNIS Reais Auditados}

\subsection{CASO 1: O PEXT que Valia R\$ 200.000}
...
\textbf{Resolução:}
\begin{enumerate}
    \item PEXT: RAC com CTPS + FGTS → DEFERIDO (45 dias)
    \item PREC-MENOR-MIN: Correção com contracheques → DEFERIDO (30 dias)
    \item Descoberta adicional: Soldador (2008-2012) → PPP → 4 anos × 1,4 = 5,6 anos
\end{enumerate}
...
RMI & --- & R\$ 4.800/mês \\
\hline
\textbf{Ganho antecipação} & & \textbf{R\$ 172.800} \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    \item 3 requerimentos online → TODOS travaram
    \item Atendimento presencial → Servidor não resolveu
...
\textbf{Perfil:} Marco, 62 anos, aposentado desde 2020 com RMI R\$ 2.900/mês

\textbf{Indicador:} PVIN-REC-PROC-TRAB

\textbf{Descoberta:} Processo trabalhista de 2019 reconheceu 4 anos de vínculo clandestino (2010-2014) com salários de R\$ 4.500/mês

\textbf{Impacto:}
\begin{itemize}
    \item +4 anos de tempo
    \item PBC recalculado
    \item Nova RMI: R\$ 3.580/mês (+R\$ 680)
\end{itemize}

\textbf{Resultado:}
\begin{itemize}
    \item Retroativo (48 meses): R\$ 32.640
    \item Ganho vitalício (20 anos): R\$ 163.200
    \item \textbf{TOTAL: R\$ 195.840}
\end{itemize}
...
\textbf{ETAPA 1: OBTENÇÃO DO CNIS}
\begin{itemize}
...
\begin{itemize}
    \item[$\square$] PEXT identificados → Documentação solicitada
    \item[$\square$] PREC-MENOR-MIN identificados → Contracheques solicitados
    \item[$\square$] PSC-MEN-SM-EC103 → Complementação calculada
    \item[$\square$] PDESFAZ-AJ-EC103 → Estratégia judicial avaliada
    \item[$\square$] PEMP-CAD → Situação cadastral verificada
    \item[$\square$] PVIN-REC-PROC-TRAB → Sentença obtida
    \item[$\square$] IEAN → PPP solicitado
\end{itemize}

\textbf{ETAPA 4: DOCUMENTAÇÃO (Tema 1124)}
\begin{itemize}
