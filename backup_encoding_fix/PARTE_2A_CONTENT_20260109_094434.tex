...
\begin{itemize}
    ...
    \item[\xmark] Tema 1300 STF (coeficiente AIP) --- 6$\times$5 REJEITADO
...
\textbf{E aí você perde R\$ 50,00.000,00 do seu cliente.}

Porque aquele CNIS que parecia ``ok'' tinha \textbf{3 indicadores críticos} na página 7 que você não viu. Um \textbf{PEXT} (vínculo extemporâneo) de 2 anos que estava sendo ignorado. Um \textbf{PREC-MENOR-MIN} que reduzia o PBC em R\$ 400,00/mês. Um \textbf{PDESFAZ-AJ-EC103} que bloquearia a concessão do benefício.
...
\begin{itemize}
    \item Tempo de contribuição $\rightarrow$ CNIS
    \item Salários para PBC $\rightarrow$ CNIS
    \item Carência $\rightarrow$ CNIS
    \item Qualidade de segurado $\rightarrow$ CNIS
    \item Atividades concomitantes $\rightarrow$ CNIS
    \item Afastamentos $\rightarrow$ CNIS
\end{itemize}
...
\begin{itemize}
    ...
    \item Salários com zeros a menos (R\$ 150,00 ao invés de R\$ 1.500,00)
...
\begin{itemize}
    \item[\xmark] Tempo não conta $\rightarrow$ O período PEXT é ignorado no cálculo
...
\begin{itemize}
    ...
    \item RMI estimada: R\$ 3,00.500,00 (em 2029)
\end{itemize}
...
\begin{itemize}
    ...
    \item RMI: R\$ 4,00.500,00 (hoje)
    \item \textbf{Ganho vitalício:} \textasciitilde R\$ 240,00.000,00
\end{itemize}
...
\begin{enumerate}
    \item Acesse Meu INSS $\rightarrow$ Serviços $\rightarrow$ Acerto de Vínculo (RAC)
...
Ao protocolar RAC para PEXT, certifique-se de apresentar DOCUMENTAÇO COMPLETA no requerimento administrativo.
...
\begin{itemize}
    ...
    \item \textbf{Fundamento:} Art. 55, \S 3\textsuperscript{o} da Lei 8.213/91
...
\begin{teseRevisional}
Salários abaixo do mínimo constitucional são ILEGAIS (CF/88, Art. 7\textsuperscript{o}, IV).

Correção pode aumentar PBC em 15-30\% e RMI em R\$ 200,00-500/mês.
\end{teseRevisional}
...
\textbf{Via Administrativa:}
\begin{enumerate}
    \item Acesse Meu INSS $\rightarrow$ Serviços $\rightarrow$ Revisão de Remunerações
...
Valor mínimo 2026: R\$ 1,00.621,00 (confirmado)
