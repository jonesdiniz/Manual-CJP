...
Coeficiente = 60% + 2% x (34 - 15) = 60% + 38% = 98%
RMI = R$ 6.000 x 0,98 = R$ 5.880,00
\end{verbatim}
...
\textbf{AÇO CJP:} SEMPRE calcule o Fator antes de recomendar Pedágio 50\%. Se Fator $<$ 0,85, evite.
\end{armadilha}
...
\begin{itemize}
    ...
    \item Pedágio: 3 $\times$ 100\% = 3 anos
...
\begin{verbatim}
SB = R$ 6.200,00
RMI = R$ 6.200 x 100% = R$ 6.200,00 (INTEGRAL)
\end{verbatim}
...
\begin{verbatim}
SB = R$ 4.500,00
Coeficiente = 60% + 2% x (18 - 15) = 60% + 6% = 66%
RMI = R$ 4.500 x 0,66 = R$ 2.970,00
\end{verbatim}
...
\subsubsection{Perfil 1: João, 62 anos, 32 anos de contribuição, SB = R\$ 7.000,00}
...
\hline
Ped. 100\% & \cmark & Daqui 3 anos & R\$ 7.000,00 & \textbf{Melhor opção!} \\
\hline
...
\subsubsection{Perfil 2: Maria, 58 anos, 33 anos de contribuição, SB = R\$ 6.000,00}
...
\textbf{Regra} & \textbf{Elegível?} & \textbf{Data} & \textbf{RMI} & \textbf{Observação} \\
\hline
Pontos & \cmark & 1 ano & R\$ 5.880,00 & 59 + 34 = 93 pontos \\
\hline
Ped. 50\% & \cmark & Agora & R\$ 4.800,00 & Fator Prev. baixo \\
\hline
Ped. 100\% & \xmark & --- & --- & Precisa 57 (tem), falta tempo \\
\hline
Idade Prog. & \cmark & 1,5 anos & R\$ 5.760,00 & 59,5 + 34,5 tempo \\
\hline
...
\subsubsection{Perfil 3: Carlos, 66 anos, 18 anos de contribuição, SB = R\$ 4.500,00}
...
\hline
Permanente & \cmark & Agora & R\$ 2.970,00 & \textbf{Única opção} \\
\hline
\end{tabularx}
\end{center}
...
\section{Fluxograma Master: Do PBC à Escolha da Regra}

O processo completo de cálculo segue este fluxo decisório:

\begin{estrategiaCJP}
\textbf{FLUXOGRAMA MASTER: CÁLCULO COMPLETO}

\textbf{INÍCIO:} Cliente procura consultoria em 2026

$\downarrow$

\textbf{ETAPA 1: COLETA DE DADOS}
\begin{itemize}
    \item CNIS atualizado
    \item Data de nascimento
    \item Tempo atual de contribuição
    \item Salários históricos
\end{itemize}

$\downarrow$

\textbf{ETAPA 2: AUDITORIA INICIAL}
\begin{itemize}
    \item Vínculos corretos?
    \item RACs pendentes?
    \item Tempo especial?
\end{itemize}
...
\textbf{ETAPA 3: IDENTIFICAR PBC}
\begin{itemize}
    \item Contribuía em 2019?
    \item DER prevista quando?
    \item Qual Era aplicável?
\end{itemize}

$\downarrow$

\textbf{ETAPA 4: CALCULAR SB}
\begin{itemize}
    \item Listar contribuições
    \item Aplicar PBC correto (80\% ou 100\%)
    \item Calcular com divisor (108 ou real)
    \item SB = Soma $\div$ Divisor
\end{itemize}

$\downarrow$

\textbf{ETAPA 5: TESTAR AS 5 REGRAS}
\begin{itemize}
    \item[$\square$] Pontos (103H/93M em 2026)
    \item[$\square$] Pedágio 50\%
    \item[$\square$] Pedágio 100\%
    \item[$\square$] Idade Progressiva (64,5H/59,5M)
    \item[$\square$] Permanente (65H/62M)
\end{itemize}

$\downarrow$

\textbf{ETAPA 6: COMPARAR RESULTADOS}
\begin{itemize}
    \item Qual dá melhor RMI?
    \item Qual é mais rápida?
    \item Quais os trade-offs?
\end{itemize}

$\downarrow$

\textbf{RESULTADO:} Recomendação Final com Justificativa Técnica
\end{estrategiaCJP}

\section{Checklist Executivo de Cálculo PBC/SB}

\begin{acaoImediata}
\textbf{CHECKLIST MASTER --- CÁLCULO PBC E SB (2026)}

\textbf{FASE 1: COLETA E VALIDAÇO}
\begin{itemize}
    \item[$\square$] CNIS atualizado (máximo 30 dias)
    \item[$\square$] Data de nascimento confirmada
    \item[$\square$] Tempo total em 13/11/2019 calculado
    \item[$\square$] Períodos de acertos (RAC) incluídos
    \item[$\square$] Tempo especial convertido (se aplicável)
\end{itemize}

\textbf{FASE 2: IDENTIFICAÇO DO PBC}
\begin{itemize}
    \item[$\square$] Cliente contribuía em 13/11/2019?
    \item[$\square$] Se SIM + regra com PBC 80\%: Usar 80\% maiores
    \item[$\square$] Se NÃO ou regra 100\%: Usar todos os salários
    \item[$\square$] DIB no Vácuo (2019-2022)? Considerar revisão Lei 14.331
\end{itemize}

\textbf{FASE 3: CÁLCULO DO SB}
\begin{itemize}
    \item[$\square$] Salários listados e atualizados (INPC)
    \item[$\square$] Descarte de 20\% menores (se PBC 80\%)
    \item[$\square$] Soma total calculada
    \item[$\square$] Divisor identificado (108 ou real)
    \item[$\square$] SB calculado: R\$ \_\_\_\_\_\_\_\_\_\_
\end{itemize}

\textbf{FASE 4: TESTE DAS 5 REGRAS}
\begin{itemize}
    \item[$\square$] Pontos: \_\_\_ pontos (precisa 103H/93M em 2026)
    \item[$\square$] Pedágio 50\%: Faltava $\leq$2 anos em 2019?
    \item[$\square$] Pedágio 100\%: Tempo + pedágio + idade (60H/57M)
    \item[$\square$] Idade Progressiva: 64,5H/59,5M + 35/30 tempo
    \item[$\square$] Permanente: 65H/62M + 15 anos
\end{itemize}

\textbf{FASE 5: COMPARAÇO E ESCOLHA}
\begin{itemize}
    \item[$\square$] Tabela comparativa montada
    \item[$\square$] Melhor RMI identificada
    \item[$\square$] Trade-offs analisados
    \item[$\square$] Recomendação final definida
\end{itemize}
\end{acaoImediata}
